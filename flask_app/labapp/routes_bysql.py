...
def index():
    ...
    return render_template('index.html', title='Whitesquare', pname='HOME', imgs=imgs, subjs=subjs)


# Обработка запроса к странице contact.html
@app.route('/contact')
def contact():
    return render_template('contact.html', title='Whitesquare', pname='CONTACT')
...
@app.route('/api/nameteam/<int:id>',methods=['GET'])
def get_name_team(id):
    response = dbservice.get_info_team(id)
    return json_response(response)
